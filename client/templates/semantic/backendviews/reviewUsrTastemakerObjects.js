...
Template.reviewUsrTastemakerObjects.helpers({
...
Template.reviewUsrTastemakerObjects.events({
    ...
    "click #updateUserTastemakerObject": function(event) {
    	//console.log('THIS IS THE user object delete was clicked for: ');
    	//console.log(this);
	   
	    var userUpdateCheck = confirm("Are you sure you want to update this user's tastemaker object: '"+this.profile.name+"'?");
	    if(userUpdateCheck == true) {
	        Meteor.call('updateUserTastemakerObject', this._id, function(error,result){
	            if(error){
	              return toastr.error(error.reason);
	            }
	            else{
	                // do something with result
	              return toastr.success('Successfully updated the tastemaker objects for that user!');
	            };
	        });
	    } 
	    else {
...
Template.reviewUsrTastemakerObjects.onCreated(function() {
	var self = this;
