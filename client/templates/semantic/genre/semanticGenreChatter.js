var genreChatterContext = new ReactiveVar(null);
var genreChatterMsgsLoaded = new ReactiveVar(false);

Template.semanticGenreChatter.helpers({
    messagesLoaded: function() {
        return genreChatterMsgsLoaded.get();
    },
    messagesForGenreChatter: function() {
...
Template.semanticGenreChatter.events({
    "click #gc-btn-sendMessage": function(event) {
    	//console.log('*********************** INSIDE GENRE CHATTERRRRRRR --- CLICKED SEND BUTTON!!!');
    	var genreChatterGroup = genreChatterContext.get().context.params._id+"_genre_group";
    	...
    	var content = $('#gc-input-chatMessage').val().trim();
    	if(!_.isEmpty(content)) //sanity check to see that user selected is the user being sent to 
    	{
    		...
    		Meteor.call('insertNewArtistGenreGroupMessage', sender, senderName, senderProfileID, genreChatterGroup, timestamp, content, function(error,result){
			    if(error){
    ...
    "keydown #gc-input-chatMessage": function(event) {
    	//console.log('this is the KEYDOWN event');
    	//console.log(event);
    	if(event.keyCode === 13) //hit enter
    	{
    		$("#gc-btn-sendMessage").click();
    	}
    },
    "focus #gc-input-chatMessage": function(event){
    	Meteor.setTimeout(animateToBottomOfChat, 100);
    }
});

//NOT USING TEMPLATE LEVEL SUBSCRIPTIONS
Template.semanticGenreChatter.onCreated(function() {
    var self = this;
    self.autorun(function() {
        ...
        //console.log(context);
        genreChatterContext.set(context);
        //artistsSubLoaded.set(false);
        //console.log("THIS IS THE PROFILE CONTEXT REACTIVE VAAAAAAAAAAAAAAAAAR: ");
        //console.log(genreChatterContext.get());
        Session.setDefault(context.params._id+'_aCursor', 0);
        /*self.subscribe('artistObjectForProfilePage', genreChatterContext.get().params._id, {onReady: artistExists});
        self.subscribe('allSongsForSpecificArtist', artistProfileContext.get().params._id, Session.get(genreChatterContext.get().params._id+'_aCursor'));
        ...
        if(!_.isUndefined(Session.get(genreChatterContext.get().params._id+'_artObj')))
        {
            self.subscribe("counterForArtistSpecificSongs", genreChatterContext.get().params._id);
            Session.set(genreChatterContext.get().params._id+'_sc', Counts.get('songCountForArtistSpecificSongs'));
            calculateArtistPercentage();
        }*/
        self.subscribe('messagesForEntity', genreChatterContext.get().params._id+"_genre_group", {onReady: genreMsgsLoaded}); //subscribe to messages for this artist page
        Session.set(genreChatterContext.get().params._id+"_mc", Messages.find({}).count());
    });
});

function genreMsgsLoaded(){
    //console.log("ARTIST MESSSSSSSSSSSSSSSSSSAGES Loaded!!!");
    genreChatterMsgsLoaded.set(true);
}

function animateToBottomOfChat() {
    //console.log('THIS IS THE chatbox last: ');
    //console.log($('#chatbox').find('.message').last()[0]);
    $('.genreChatterMessages').animate({ scrollTop: $('.genreChatterMessages')[0].scrollHeight}, 500)
}
