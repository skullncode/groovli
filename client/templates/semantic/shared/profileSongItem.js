if (Meteor.isClient) {   
  Template.profileSongItem.helpers({
    ...
    sharedByDeetsForGlobalItem: function() {
      ...
      while(shareCounter < this.sharedBy.length)
      {
        ...
        globalIDsThatSharedThisSong.push({strangerProfID: this.sharedBy[shareCounter]._id, strangerID: this.sharedBy[shareCounter].uid, strangerName: this.sharedBy[shareCounter].uname, strangerTimestamp: new moment(this.sharedBy[shareCounter].systemDate * 1000).format('llll')});
  ...
  /*
  Template.profileSongItem.events({
