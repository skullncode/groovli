Template.genreChatter.rendered = function() {
	...
	Meteor.subscribe('messages', Router.current().params._name+"_genre_group"); //subscribe to messages for this artist page
};

Template.genreChatter.helpers({
	userList: function() 
	{
	...
	formattedTimestamp: function() {
		if(this.to === Router.current().params._name+"_genre_group")
		{
...
Template.genreChatter.events({
    "click #btn-sendMessage": function(event) {
    	//console.log('CLICKED SEND BUTTON!!!');
    	var genreChatterGroup = Router.current().params._name+"_genre_group";
    	...
    	if(!_.isEmpty(content)) //sanity check to see that user selected is the user being sent to 
    	{
    		...
    		Meteor.call('insertNewArtistGroupMessage', sender, senderName, senderProfileID, genreChatterGroup, timestamp, content, function(error,result){
			    if(error){
