...
function getLatestDTForSpecificUser(stories, selectedUser) {
	//console.log('CHECKING FOR LATEST DT for this user:');
	//console.log(selectedUser);
	Meteor.call('updateDataImportProgress',Meteor.user()._id, 0.20); //stage 1 of 5 for import
...
function remoteMobileTriggerToGetStoriesFromFBOnly(userObj, dt, pagedQuery) {
	var updatedFBGetURL = '';
	if(_.isEmpty(pagedQuery))
	{
		if(dt === 0) // if last date time stamp is 0 then nothing has ever been brought in for this user; so just remove the SINCE limiter
			updatedFBGetURL = fbGraphURL.replace('#POST_LIMIT#',fbLimit).replace('&since=#LAST_TIME#','').replace('#TOKEN#',userObj.services.facebook.accessToken);
		else
			updatedFBGetURL = fbGraphURL.replace('#POST_LIMIT#',fbLimit).replace('#LAST_TIME#',dt).replace('#TOKEN#',userObj.services.facebook.accessToken);
	}
	else
	{
		...
		function(error, result) {
			if(!error && result.statusCode === 200) {
				...
				while(counter < result.data.data.length)
				{
					...
					if(!_.isUndefined(result.data.data[counter].link) && (result.data.data[counter].link.indexOf('youtube.com') >= 0 || result.data.data[counter].link.indexOf('youtu.be') >= 0))
					{
						console.log('#1 THIS IS A YOUTUBE LINK: '+result.data.data[counter].link);
						console.log('MSG with LINK: '+result.data.data[counter].message);
						if(result.data.data[counter].link.indexOf('youtu.be') >= 0) //fix youtu.be links to be normal youtube links
						{
							var foundYTLink = {
								...
								storyLink: result.data.data[counter].link.replace('youtu.be/','www.youtube.com/watch?v='),
				...
				performYoutubeValidation(foundYTLinkCollection, "mobile", userObj);
...
function getStoriesFromFBOnly(dt, pagedQuery) {
	Meteor.call('updateDataImportProgress',Meteor.user()._id, 0.40); //stage 2 of 5 for data import
	var updatedFBGetURL = '';
	if(_.isEmpty(pagedQuery))
	{
		if(dt === 0) // if last date time stamp is 0 then nothing has ever been brought in for this user; so just remove the SINCE limiter
		...
		function(error, result) {
			if(!error && result.statusCode === 200) {
				...
				while(counter < result.data.data.length)
				{
					//is a valid YouTube link
					//console.log('COUNTER now is: ' + counter);
					Meteor.call('updateDataImportProgress',Meteor.user()._id, 0.45);
					if(!_.isUndefined(result.data.data[counter].link) && (result.data.data[counter].link.indexOf('youtube.com') >= 0 || result.data.data[counter].link.indexOf('youtu.be') >= 0))
					{
						console.log('#2 THIS IS A YOUTUBE LINK: '+result.data.data[counter].link);
						console.log('MSG with LINK: '+result.data.data[counter].message);
						if(result.data.data[counter].link.indexOf('youtu.be') >= 0) //fix youtu.be links to be normal youtube links
						{
							var foundYTLink = {
						...
						console.log('PUSHING THIS Item to the found result set:');
						console.log('########');
						console.log(foundYTLink);
					...
					counter++;
					Meteor.call('updateDataImportProgress',Meteor.user()._id, 0.45 + (counter / 100)); 
				}
				//console.log('Validating : ' + ytCounter + ' yt Links to check if they are songs from fB posts');
				Meteor.call('updateDataImportProgress',Meteor.user()._id, 0.50);
				performYoutubeValidation(foundYTLinkCollection, "site", null);

				//COMMENT OUT TO LIMIT GETTING MORE POSTS!!!!! ONLY FOR TESTING!!!!!!@@@@
				if(!_.isUndefined(nextPagingURL) && !_.isEmpty(nextPagingURL))
				{
					Meteor.call('updateDataImportProgress',Meteor.user()._id, 0.60); //stage 2 of 5 for data import
					getStoriesFromFBOnly(0,nextPagingURL);
				}
			}
			else// error is defined
...
function cleanTrackQueryPriorToLastFMCheck(trackQuery)
{
	...
	trackQuery = trackQuery.replace('HQ',' ');
	trackQuery = trackQuery.replace('OFFICIAL AUDIO',' ');
...
function wuzzyMatchingAnalysisForLFMData(searchTerms,lfmResults, count)
{
	...
	var result1 = 0;
	var result2 = 0;
	if(!_.isUndefined(searchTerms) && !_.isEmpty(searchTerms) && !_.isNull(searchTerms))
	{
...
function getAdditionalMissingDataFromItunesAPI(linkToBeValidated) {
	if(!_.isUndefined(linkToBeValidated))
	{
		...
		console.log('CANNOT get additional missing data from ITUNES API as linkToBeValidated is undefined');
}
...
function performLastFMValidationForYTLink(linkToBeValidated) {
	if(!_.isUndefined(linkToBeValidated))
	{
		...
		console.log('CANNOT do lastFMValidation as linkToBeValidated is undefined');
}
...
function insertNewStory(fbResult, network, ytResult)
{
	...
	{
		console.log('CANNOT insert new story as FB result is undefined');
		console.log('this is the FBResult:');
		console.log(fbResult)
	}
		
}

function performYoutubeValidation(FBYTResultSet, mode, userObject) 
{
	...
	{
		console.log("BEFORE REPLACING Story link this is the original link########################:");
		console.log(FBYTResultSet[counter].storyLink);
		if(FBYTResultSet[counter].storyLink.indexOf('attribution_link')) //special case attribution link youtube link
		{
			var x = FBYTResultSet[counter].storyLink
			vidIDSet[counter] = x.substring(x.indexOf('&u=%2Fwatch%3Fv%3D')+18, x.indexOf('%26feature'))
		}
		else //normal link and not an attribution link
		{
			vidIDSet[counter] = FBYTResultSet[counter].storyLink.substring(FBYTResultSet[counter].storyLink.indexOf('v=')+2);
			console.log("########### AFTER ONE REPLACE OPERATION THIS IS THE SONG LINK:");
			console.log(vidIDSet[counter])
			...
			console.log("########### AFTER FINAL REPLACE OPERATION THIS IS THE SONG LINK:");
			console.log(vidIDSet[counter])
		}
	...
	{
		Meteor.call('updateDataImportProgress',Meteor.user()._id, 0.65);
		...
		function(error, result) {
		  if(!error && result.statusCode === 200) {
			...
			if(YTResultList.items.length > 0)
	    	{
				...
				var matchingVidIDElement =  vidIDSet.filter(function(el){
					return el.indexOf(linkID) >= 0;
				});
				console.log('==================INSIDE YOUTUBE Validation METHOD=============================================');
				console.log("this is the matching LINK ID that we need to search for: " + linkID);
				console.log('AND THIS IS THE FULL VID ID SET: ');
				console.log(vidIDSet);
				console.log("this is the matching video element: " + matchingVidIDElement);
				var matchingVidIndex = vidIDSet.indexOf(String(matchingVidIDElement));
				console.log("THIS is the matching video index: " + matchingVidIndex);
				var fbResult = FBYTResultSet[matchingVidIndex];
				console.log('this is the fb result after getting the index of the matching video element: ');
				console.log(fbResult);
				
				/*console.log('THIS IS THE FB result BEFORE INSERTING NEW STORY: ');
				console.log(fbResult);*/
				console.log("#########and this is the FBYT Result List:")
				console.log(FBYTResultSet)
				...
				else
				{
					...
					Meteor.call('insertNewSong',fbResult, 'FB', 'YOUTUBE', mode, userObject);
					validCounter++;
	  ...
	  counter++;
	  Meteor.call('updateDataImportProgress',Meteor.user()._id, 0.65 + (counter / 100)); 
	}
	Meteor.call('updateDataImportProgress',Meteor.user()._id, 0.90); 
}

Meteor.methods({

	remoteAPITriggerToGetLatestFromFB: function(userObj) {
		if(!_.isUndefined(userObj) && !_.isEmpty(userObj))
		{
			var fbUserID = userObj.services.facebook.id;
			console.log("%%%%%%%%%%%%%%%%%%%%this is the userID: " + fbUserID); 
			var returnedStories = Stories.find({}).fetch();
			remoteMobileTriggerToGetStoriesFromFBOnly(userObj, getLatestDTForSpecificUser(returnedStories, fbUserID), '');
			return "success";
		}
