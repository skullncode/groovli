...
function getStoriesFromFBOnly(dt) {
	var updatedFBGetURL = fbGraphURL.replace('#POST_LIMIT#',fbLimit).replace('#LAST_TIME#',dt).replace('#TOKEN#',Meteor.user().services.facebook.accessToken);
		...
		function(error, result) {
			if(!error && result.statusCode === 200) {
				while(counter < result.data.data.length)
				{
					...
					if(result.data.data[counter].link.indexOf('youtube.com') >= 0)
					{
...
function wuzzyMatchingAnalysisForiTunesData(searchTerms, iTunesResults, resultCount, checkingIndex)
{
	if(resultCount > 0) {
		...
		if(iTunesResults[checkingIndex].kind === 'song')
		{
			...
			//var result1 = wuzzy.jarowinkler(iTunesTitleArtist,searchTerms);
...
function wuzzyMatchingAnalysisForLFMData(searchTerms,lfmResults, count)
{
	...
	if(count > 1)
	{
		if(lfmResults.trackmatches.track[checkingIndex].artist !== '[unknown]')
		{
			...
			//result1 = wuzzy.jarowinkler(lfmTitleArtist,searchTerms);
		  ...
		  if(!error && result.statusCode === 200) {
		    ...
		    if(itunesResults.resultCount > 0) // skip sifting through LFM results if there are no results
		    {
		    	...
		    	while(counter < itunesResults.resultCount)
		    	{
				    ...
				    //console.log('REACHED LAST FM success: '+itunesResults.results['opensearch:Query'].searchTerms);
				    //siftThroughLastFMResults(body, socketObject);
				    ...
				    if(counter < 10)
				    {
					    ...
					    {
					    	//console.log('NOTHING found in ITUNES for this: ' + cleanedTrackDetails);
					    	//console.log('highestMatchingWuzzyFactor is: ' + highestMatchingWuzzyFactor);
					    }
				...
				if(matchFound)
				{
					...
					Meteor.call('updateTrackWithiTunesData',cleanedTrackDetails, itunesResults.results[highestMatchingIndex]);
				}
