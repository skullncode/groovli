...
Meteor.methods({
	...
	insertNewSong: function(fbResult, network, linkType){
		...
		var newSong = {
			...
			sharedBy: [{'uid':fbResult.uid, 'systemDate':fbResult.systemDate, 'msg':fbResult.msgWithStory}],
	...
	updateAEC: function(sid, type, count){
		//console.log('INSIDE THE UPDATE LISTEN couNT METHOD FOR SONG SERVER CONTROLLER');
		...
		Songs.update({'sl': searchString}, {$set: { aeCount: count }}, function(error) {
	      if (error) {
	        // display the error to the user
	        console.log(error.reason);
	      }
