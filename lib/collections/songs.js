...
//This function has been moved to the landing page itself as it seemed to work better
/*function getSpotifyAlbumArtForSong(sArtist, sTitle){
	...
	console.log("this is the query going to be used to get the spotify album art:");
	console.log(spotifyAlbumArtURL);
	return Meteor.http.get(spotifyAlbumArtURL);
...
Meteor.methods({
	...
	insertNewSong: function(fbResult, network, linkType, initiator, uObj){
		if(!_.isUndefined(fbResult))
		{
			...
			else if(initiator == "mobile")
			{
				var newSong = {
					...
					sa: fbResult.storyTitle,
					st: fbResult.storyTitle,
					sharedBy: [{'uid':fbResult.uid, 'systemDate':fbResult.systemDate, 'msg':fbResult.msgWithStory, 'uname':uObj.services.facebook.name, '_id':uObj._id}],
					iTunesLargeAlbumArt: cleanTrackQueryPriorToLastFMCheck(fbResult.storyTitle), // in order to find the Song after itunes Validation completes
					LFMLargeAlbumArt: cleanTrackQueryPriorToLastFMCheck(fbResult.storyTitle), // in order to find the Song after lastFm Validation completes
					songSearchText: cleanTrackQueryPriorToLastFMCheck(fbResult.storyTitle),
					iTunesValid: 'PENDING',
			...
			if (!_.isUndefined(songWithSameID)) {
			  ...
			  {
				  //if Song already exists we need to update the SharedBy to include that person's id ALSO:
				var sharedByEntry = {'uid':fbResult.uid, 'systemDate':fbResult.systemDate, 'msg':fbResult.msgWithStory, 'uname':uObj.services.facebook.name, '_id':uObj._id}
			  }
	...
	getLast10TrendingSongsForLandingPage: function() {
      	var y = Songs.find({$or: [{iTunesValid:'VALID'},{LFMValid:'VALID'},{manualApproval:'VALID'}], $and: [{'cover': {'$ne': true}}, {'iTunesLargeAlbumArt': {'$ne': ''}}, {'iTunesLargeAlbumArt': {'$ne': 'none'}}, {'iTunesLargeAlbumArt': { $regex: /^http:/i }}]}, {sort: {'sharedBy.systemDate': -1}, limit:80,
      	fields: {
		...
		_.each(y, function (z){
			if(!_.contains(onlyLinksForListens, z.sl))
			{
				...
				if((listenResult > 2) || (ratingResult > 1))
				{
	...
	getLast20SongsForLandingPage: function() {
      	var y = Listens.find({}, {sort: {'timestamp': -1}, limit:15}).fetch();
		...
		var x = Songs.find({$and: [{'cover': {'$ne': true}}, {'iTunesLargeAlbumArt': {'$ne': ''}}, {'iTunesLargeAlbumArt': {'$ne': 'none'}}, {'iTunesLargeAlbumArt': { $regex: /^http:/i }}, {'sl': {$in: randomSample}}]}, {sort: {'sharedBy.systemDate': -1}, limit:5, 
			fields: {
				"iTunesLargeAlbumArt": 1,
        		"sa": 1,
        		"st": 1,
		...
		/*var updatedLandingSongList = [];
		_.each(x, function(songObj){
			var y = getSpotifyAlbumArtForSong(songObj.sa, songObj.st);
			console.log("this is the result from the spotify request: ");
			console.log(y.statusCode);
			if(!_.isUndefined(y.data.tracks.items[0]) && !_.isEmpty(y.data.tracks.items[0]) && y.statusCode == 200)
			{
				//console.log("this happens to be the album art for this song: " + songObj.sa + " " + songObj.st);
				//console.log(y.data.tracks.items[0].album.images[2].url);
				updatedLandingSongList.push({sa: songObj.sa, st: songObj.st, spotifyAlbumArtURL: y.data.tracks.items[0].album.images[2].url})
				//return result.data.tracks.items[0].album.images[2].url;
			}			
		});*/

		return x;
		//return updatedLandingSongList;
	},
	/*
	...
	manuallyDeleteSong: function(songID) {
		Songs.remove({_id: songID}, function(error, result){
			if(error) {
				console.log('encountered an error while trying to delete the chosen song: ' + songID);
			}
			else{
				console.log('succesfully deleted the chosen song - ' + songID);
			}
		});
	},
	...
	getTopSongsBasedOnListenCount: function(cursorSkip){
	...
	getListOfStatsForYearInReview: function(userID){
		...
		if(!_.isUndefined(userObj) && !_.isUndefined(userObj.services) && !_.isUndefined(userObj.services.facebook))
		{
			songList = Songs.find({'sharedBy.uid': String(userObj.services.facebook.id), $or: [{iTunesValid:'VALID'},{LFMValid:'VALID'},{manualApproval:'VALID'}]}).fetch();
			songCount = songList.length;
		}

		if(!_.isUndefined(userObj) && !_.isUndefined(userObj.services) && !_.isUndefined(userObj.services.facebook))
		{
			var faveBandGenres = Meteor.call('getFaveBandList', String(userObj.services.facebook.id));
			var timespan = Meteor.call('getSongDateRangeForSpecificUser', String(userObj.services.facebook.id));
			//console.log("########### this is the timespan for the year in review: ");
			//console.log(timespan);
			firstName = String(userObj.services.facebook.first_name);
		}

		_.each(songList, function(x){
			//console.log('calculating total song time span');
			//console.log(x.sa + ' - ' + x.st);
			//console.log("track duration: " + x.trackDuration);
			//console.log('current total song time: ' + totalSongTime);
			if(_.isNumber(x.trackDuration))
			{
				var timeInMins = (x.trackDuration / 1000) / 60;
				totalSongTime += timeInMins
			}
		});

		totalSongTime = Math.round(totalSongTime);

		//console.log('######################this is the result of the timespan:');
		//console.log(timespan);

		var songSpan = calculateSongTimeSpan(timespan);//0;

		//var bandAndGenres = getBandAndGenresSeparately(faveBandGenres);

		//console.log("thIS IS THe basdfand andasdf genres: ");
		var topBand = ""; 
		var topGenre = "";

		if(!_.isUndefined(faveBandGenres) && !_.isUndefined(faveBandGenres[0][0]))
		{
			topBand = faveBandGenres[0][0].sa;
			topGenre = faveBandGenres[1][0].name;
		}

		var socialText = createSocialShareText(songCount, songSpan, topBand, topGenre);

		//console.log("user object is: ");
		//console.log(userObj);

		return {"firstName": firstName, "socialText": socialText, "totalPlayTime": totalSongTime, "songTimespan": songSpan, "totalSongCount": songCount, "faveBandGenres": faveBandGenres};
	}
	...
	//MOBILE SONG methods
	/*getMyGroovsForMobileApp: function(userID, cursorSkip) {
		var options = {
			fields: {
			"songSearchText": 0, 
			"aeCount": 0, 
		...
		sort: {'sharedBy.systemDate': -1 },
		skip: Number(cursorSkip),
		limit: 50};
		var myGroovsForMobile = Songs.find({"sharedBy._id": String(userID),$or: [{iTunesValid:'VALID'},{LFMValid:'VALID'},{manualApproval:'VALID'}]}, options).fetch();
		return myGroovsForMobile;
	},*/
});

function calculateSongTimeSpan(tspan){
	if(!_.isEmpty(tspan))
	{
		if(tspan.length == 2)
		{
			if(!_.isUndefined(tspan[0])  && !_.isUndefined(tspan[1]))
			{
				if(tspan[0] !== tspan[1])
				{
					var diff = tspan[1] - tspan[0];
					if(diff == 1)
					{
						return 1;
					}
...
function createSocialShareText(songCount, timeSpan, topArtistName, topGenreName)
{
	var shareTxt = "";
	//I%27ve shared {{songCount}} songs over the span of {{totalTimeSpan}} {{#if yearCountPluralRequired}}years{{else}}year{{/if}}{{artistAndGenreShareText}} %23groovli &#64;groovliapp %23nowplaying
	shareTxt += "I%27ve shared " + songCount + " songs over the span of " + timeSpan;
	...
	if(!_.isNull(topArtistName))
	{
		//shareTxt = "";
		shareTxt += " - my top artist is " + topArtistName;
	}

	if(!_.isNull(topGenreName))
	{
		shareTxt += " - my top genre is " + topGenreName;
	}

	shareTxt += " %23groovli";// %23groovliapp %23nowplaying";
	//console.log('this is the share text: ' + shareTxt);
