...
Meteor.methods({
	...
	getListOfStatsForYearInReview: function(userID){
		...
		if(!_.isUndefined(timespan))
		{
			...
			else if(timespan.length = 2 && timespan[0] == timespan[1])
			{
				songSpan = 1;
			}
			else if(timespan.length = 2 && timespan[0] !== timespan[1])
			{
				songSpan = timespan[1] - timespan[0]
			}
		}

		return {"totalPlayTime": totalSongTime, "songTimespan": songSpan, "totalSongCount": songCount, "faveBandGenres": faveBandGenres};
	}
