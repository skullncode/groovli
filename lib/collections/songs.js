...
Meteor.methods({
	...
	getLast20SongsForLandingPage: function() {
      	var y = Listens.find({}, {sort: {'timestamp': -1}, limit:30}).fetch();
		...
		var x = Songs.find({$and: [{'cover': {'$ne': true}}, {'LFMLargeAlbumArt': {'$ne': ''}}, {'LFMLargeAlbumArt': {'$ne': 'none'}}, {'LFMLargeAlbumArt': { $regex: /^http:/i }}, {'sl': {$in: onlyLinksForListens}}]}, {sort: {'sharedBy.systemDate': -1}, limit:6, 
			fields: {
				"LFMLargeAlbumArt": 1,
        		"sa": 1,
        		"st": 1,
