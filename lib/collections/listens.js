...
Meteor.methods({
	...
	getListenHistoryForUser: function(uid){
		...
		var lh = Listens.find({'uid': uid}, {sort: {timestamp: -1}, limit:150}).fetch();
		...
		_.each(lh, function(x) {
			...
			//console.log(lid);
			if(!_.isEmpty(lid) && cleanedCounter < 50) //changed from 100 to 50 to only get last 50 recent listens
			{
				cleanedLH.push(x);
				cleanedCounter++;
			}
