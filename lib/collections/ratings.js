...
Meteor.methods({
	...
	getAverageRatingForSong: function(linkSID, linktype){
		...
		//else
			//console.log('NOT returning anything as no RATINGS exist for this song yet!');
		/*if(ratings.length > 0)
		{
			console.log('RATINGS FOUND IN SERVER FOR THIS SONG: ' + searchString);
			var counter = 0;
			var avgRating = 0;
			while(counter < ratings.length)
			{
				avgRating += ratings[counter].rating;
				counter++;
			}
			avgRating = avgRating / ratings.length;
			var comboRatingLength = avgRating + "|||" + ratings.length+ "|||" + linkSID;
			var socketio = socketObject; // tacke out socket instance from the app container
			socketio.sockets.emit('song.averageRating', comboRatingLength); // emit an event for all connected clients
		}
		else
		{
			console.log('DID NOT FIND ANY RATINGS FOR THIS SONG: ' + searchString);
			console.log(err);
			var comboRatingLength = 0 + "|||" + 0 + "|||" + linkSID;
			var socketio = socketObject; // tacke out socket instance from the app container
			socketio.sockets.emit('song.averageRating', comboRatingLength); // emit an event for all connected clients
		}*/
	}
