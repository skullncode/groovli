...
if (Meteor.isServer) {
  ...
  JsonRoutes.add("get", "/api/mobile/usrprof/:_usrid/:_accessToken", function (req, res, next) {
    var userID = req.params._usrid;
    var axToken = req.params._accessToken
    console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
    console.log("THIS IS THE user ID: ");
    console.log(userID);
    console.log("and this is the token:");
    console.log(axToken)
    var userProfileObj = Meteor.users.findOne({_id: String(userID)});
    console.log("THIS IS THE found USER PROFILE OBJECT: ");
    console.log(userProfileObj.mobileFBToken);
    if(!_.isEmpty(userProfileObj) && axToken == userProfileObj.mobileFBToken)
    {
      ...
      Meteor.call('getFaveBandList', String(userProfileObj.services.facebook.id), function(error, result) {
          ...
          else
          {
            ...
            JsonRoutes.sendResult(res, 200, {"sharedSongCount": songCount, "ratingCount": ratingCount, "listenCount": listenCount, "followingCount": followingCount, "followedByCount": followedByCount, "topBands": result[0], "topGenres": result[1], "userObj": userProfileObj});
          }
        });
      console.log("##############################################");
    }
