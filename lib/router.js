...
SEO.setDefaults({
  title: 'Groovli',
  description: 'your social jukebox',
  meta: {
    'property="og:type"': 'website',
    'property="og:title"': 'your social jukebox - Groovli',
    'property="og:image"': 'https://groovli.com/images/yirogimage.png',
    //'property="og:image"': 'http://localhost:3000/images/yirogimage.png',
    'property="og:site_name"': 'Groovli',
    'name="twitter:card"': 'your social jukebox - Groovli',
    'name="twitter:site"': '@groovliapp'
  }
});
...
//eagle eye - see top songs
FlowRouter.route('/ee_topsongs/', {
  triggersEnter: [requireLogin],
  action: function(params) {
    //console.log("Yeah! going to load the top songs now!!");
    Session.set('cr', '/ee_topsongs');
    BlazeLayout.render( 'semanticProfileLayout', { 
      header: 'semanticHeader',
      main: 'topSongs',
      //sidebar: 'messagingSidebar',
...
//marketing route - year in review
FlowRouter.route('/yir/', {
  triggersEnter: [checkLoginForYearInReview],
  action: function(params) {
    //console.log("Yeah! going to load the top songs now!!");
    Session.set('cr', '/yir');
    BlazeLayout.render( 'landingLayout', { 
...
//marketing route - year in review
FlowRouter.route('/yir/:_id', {
  //triggersEnter: [requireLogin],
  action: function(params) {
    //console.log("Yeah! Going to the Profile page for:", params._id);
    Session.set('cr', '/yir/:_id');
    BlazeLayout.render( 'landingLayout', { 
      //header: 'semanticHeader',
      main: 'yearInReviewDetail',
      //footer: 'semanticFooter'
    }); 
  },
  name: 'yearinreviewDetail'
});
...
function checkLoginForYearInReview() {
  ...
  if (!_.isNull(Meteor.user()) && !_.isUndefined(Meteor.user())) {
    ...
    if (Meteor.loggingIn()) {
      ...
      //console.log('NOT LOGGED IN / USER DOES NOT EXIST!!!');
      //console.log(Meteor.user());
      FlowRouter.go('/yir');
  }
}
...
function triggerRemoteFBUpdateForBrandNewUser(emailForNewUser){
  var userUpdateObj = Meteor.users.find({'services.facebook.email': String(emailForNewUser)}).fetch();
...
if (Meteor.isServer) {
  ...
  JsonRoutes.add("post", "/api/mobile/deets/:_email/:_accessToken/", function (req, res, next) {
    ...
    if(_.isEmpty(userUpdateObj)){//need to take care of users who have never ever used Groovli
      var usernameForNewUser = email.substring(0,email.indexOf('@'));
      console.log("DID NOT FIND ANY USER WITH MATCHING EMAIL ADDRESS!!!! going to try and create a user in the background!!!!");
      console.log("GOING TO attempt to create a user with this username: " + usernameForNewUser);
      var currentDate = new Date();
      var newlyCreatedUserID = Meteor.users.insert({
         services:{"facebook":{"id": "00000000","email":email,"name":usernameForNewUser}},
         profile:{"name":usernameForNewUser},
         tastemakers: [],
         "roles":["tester"],
         "status":{"online":false, "lastLogin":{"date":currentDate}, "ipAddr":"mobileApp"},
         mobileFBToken: userAccessToken
      });
      var newlyCreatedUser = Meteor.users.find({'services.facebook.email': String(email)}).fetch();
      Meteor.call('updateUserDataForNewlyCreatedUser', newlyCreatedUserID, String(email), userAccessToken, function(error, result) {
          if(error)
          {
            console.log('Encountered error while trying to update internal fb details!');
            JsonRoutes.sendResult(res, 200, {"result": "Encountered error while trying to update internal fb details of brand new user"});
          }
          else
          {
            triggerRemoteFBUpdateForBrandNewUser(email);
            console.log('SUCCESSFULLY updated this users internal fb details:!!');
            console.log(newlyCreatedUserID)
            console.log("#####SUCCESSFULLY CREATED a new user from scratch!!!!!!!!!" + JSON.stringify(newlyCreatedUser));
            var finalGlobalSel = addSongValidatorsToSelector(getMongoSelectorForGlobal(newlyCreatedUser[0]), "global");
            var globalSongCount = Songs.find(finalGlobalSel).count();
            JsonRoutes.sendResult(res, 200, {"result": String(newlyCreatedUserID), "brandNewUser": "true", "myGroovsCount": 0, "tastemakerSongCount": 0, "globalSongCount": globalSongCount});
          }
        });
    }
    else if(!_.isEmpty(userUpdateObj)) //this ONLY takes care of users who have already used Groovli once
    {
      console.log("Found this user in the DB: "+ JSON.stringify(userUpdateObj));
      ...
      // token needs to be updated
      {
        ...
        Meteor.call('updateMobileFBToken', userUpdateObj[0], userAccessToken, function(error, result) {
          ...
          else
          {
            ...
            console.log(userUpdateObj[0])
            JsonRoutes.sendResult(res, 200, {"result": String(userUpdateObj[0]._id), "brandNewUser": "false", "myGroovsCount": mySongCount, "tastemakerSongCount": tastemakerSongCount, "globalSongCount": globalSongCount});
          }
        });
      }
      else{
        console.log("MOBILE FB token is the same and doesn't need to be updated!");
        JsonRoutes.sendResult(res, 200, {"result": "MOBILE FB token is the same and doesn't need to be updated!", "brandNewUser": "false", "myGroovsCount": mySongCount, "tastemakerSongCount": tastemakerSongCount, "globalSongCount": globalSongCount});
      }
    }    
  });
